...
​		跨进程调用需要走网络，进行网络编程，网络编程就涉及到两个协议了：

* 使用`Http`协议：
  * `Http`协议对于程序员来讲，是最方便的，不用开发服务器，有现成的，例如`tomcat`、`resin`、`weblogic`等
  * 对于服务端(指的是提供订单服务的系统)来说，服务需要通过`Servlet`或`Controller`进行暴露，因为他们要遵守`JavaEE`的`Servlet`规范。
  * 对于客户端（web门户）来说，不一定通过浏览器，需要使用`HttpClient`、`OKHttp`、`RestTemplate`、`WebClient`来发起请求。代码编写是十分复杂的。
* 使用`Http`协议存在的问题：
  * 包一层，不能直接调用`Service`
...
* 使用`Http`协议：
  ...
  * 对于客户端（web门户）来说，不一定通过浏览器，需要使用`HttpClient`、`OKHttp`、`RestTemplate`、`WebClient`来发起请求。代码编写是十分复杂的。
* 使用`Http`协议存在的问题：
  * 包一层，不能直接调用`Service`。（为了满足`JavaEE`规范，需要包一层`Servlet`）。
  * 客户端代码过于繁琐。例如使用`HttpClient`，需要`new HttpClient()`，创建`HttpGet`，`HttpPost`。不能直接表达业务含义。
  * 传输相同的数据，走`Http`协议，传输数据的数据量大。包了很多协议头，例如`Cookie`。传输数据慢，需要逐层传输，逐层解析。因为Http协议是应用层协议。

* 典型的技术实现方案：
  * `SpringCloud Feign`
  * `Hessian RPC`
* 优势：
  * Http协议是文本类型，字符串协议，跨语言平台好。大多数情况下走的都是文本，少数情况下也可以走二进制，例如下载文件。
* 注意：Http协议底层也是TCP协议

#### TCP协议

* 跨进程调用的本质是网络通信，通信的就是数据，就是参数和返回值。
* 使用TCP协议需要解决的问题：
  * 需要开发服务器，进行网络编程，使用`Socket`、`NIO`，`Netty`等。在服务器端还需要接受客户端传递的参数以及调用哪个方法的名字。还需要返回结果。
  * 需要开发客户端，没有像`HttpClient`这样的客户端了。接收服务端的返回值。
  * 自己定义应该协议，自己使用序列化的方式。
* 缺点：
  * 需要开发服务器，代码量大。
  * 客户端也需要自己开发，开发难度大，对使用者不友好。
* 优点：
  * 可以直接进行Service的调用。**`Dubbo`走的就是TCP协议**
  * 效率高，不需要频繁的进行3次握手和4次挥手。线程管理好
* 典型的技术实现方案：`Dubbo`

**HTTP和TCP两种方案都可以成为RPC跨进程调用**。
...
2. 客户端(web门户)需要解决的问题：
   ...
   * 网络通信，协议的选择
   * 传输数据，自定义传输协议，数据的序列化(`json`，`protobuf`，`hessian`)
   * 在调用者这方创建远端服务类的代理类，为原始类增加了传输数据和网络通信的功能。

3. 服务端(后台)需要解决的问题：

   * 网络通信
   * 协议的处理，解析数据，拿到客户端传递的数据。【类+方法+参数】

4. 为了实现设计目标，需要在客户端提供一个与服务端同名的类，同名的方法，只是不会照抄服务端的代码，而是通过网络来调用服务端的方法，并接收返回值进行后续处理。因此本地的同名方法需要有以下三个功能：

   * 传输数据
   * 网络通信的功能
   * 完成远端服务方法的功能(原始类的功能)

   本地的同名类其实就是一个代理类（术语：Stub），为远端的原始类（术语：Skeleton）增加额外功能。

5. 总结实现RPC需要解决的核心问题：

   * 网络通信时协议的选择，TCP/Http协议
   * 传输数据时需要自定义传输协议，**完成数据的序列化**
   * 在调用者方需要创建远端服务类的代理类。
...
​		如果有多个客户端访问同一个服务的话，为了考虑到并发的问题，就需要为服务端搭建集群了，那么最终客户端访问服务时到底访问哪个机器，就需要有一个同一个管理，这就是注册中心需要干的事。

​		所有的服务都在注册中心进行注册。后续客户端访问服务时就不直接访问服务端了，而是访问注册中心，然后由注册中心决定请求哪个机器的服务。

​		使用注册中心的优点：

* 负载均衡
* 管理服务，帮助我们判断服务端哪个是健康的，哪个是不健康的。通过心跳的方式。
* 解耦合，服务提供者和调用者之间的联系就没有了。
* 熔断机制
* 限流，保证服务的安全。
...
* 纯粹的RPC，只解决了RPC中的4个核心问题（网络通信、协议、序列化、代理）
* 是由java编写的
* Hessian RPC落伍了，但是他的序列化方式，被Dubbo还在使用。Dubbo使用的Hessian是阿里定制的Hessian Lite。

### 2.4.1 基本概念

1. Hessian是Resin服务器的伴生产品。Resin就是类似tomcat的一个服务器。
2. 基于Java编程语言设计的RPC框架，只支持Java编程语言使用（服务的调用者和提供者都是Java开发的）
3. 序列化协议是二进制的
4. 官网：http://hessian.caucho.com/

### 2.4.2 Hessian RPC的设计思想

* 服务器，使用现成的服务器，tomcat或resin。HTTP协议

* 创建服务

  * 开发Service，必须定义接口。因为调用者要做代理，JDK代理必须要使用接口。

* 传输数据，需要序列化，因此在对象类型的数据，必须实现`Serializable`接口

* 服务的发布：

  * 让调用者可以知道提供了哪些服务，以及这些服务如何访问。在Hessian中通过一个HessianServlet来实现。在web.xml配置

    ```xml
    <!--订单服务-->
    <servlet>
    	<servlet-name>hessianServlet</servlet-name>
        <servlet-class>xxxx.xxxx.HessianServlet</servlet-class>
        <init-param>
            <param-name>home-api</param-name>
            <param-value>com.zw.service.OrderService</param-value>
        </init-param>
        <init-param>
        	<param-name>home-class</param-name>
            <param-value>com.zw.service.impl.OrderServiceImpl</param-value>
        </init-param>
    </servlet>
    ...
    <!--用户服务-->
    <servlet>
    	<servlet-name>hessianServlet2</servlet-name>
        <servlet-class>xxxx.xxxx.HessianServlet</servlet-class>
        <init-param>
            <param-name>home-api</param-name>
            <param-value>com.zw.service.UserService</param-value>
        </init-param>
        <init-param>
        	<param-name>home-class</param-name>
            <param-value>com.zw.service.impl.UserServiceImpl</param-value>
        </init-param>
    </servlet>
  ...
  * 发请求的时候只能使用Post请求，因为http的二进制序列化只能走Post请求的请求体。

  * `HessianProxyFactory`做服务代理，需要两个要素：

    * 要代理的类，`OrderService.class`
    * url，通过网络连接服务端，需要知道服务端的url

  * 服务的接口需要做成公共模块，服务端和客户端都需要知道接口。
